...
**אין כרגע אף נכס Pexels באתר.** שני חסמים:

1. `PEXELS_API_KEY` לא מוגדר בפרויקט.
2. הסביבה שבה נבנה האתר חוסמת את Pexels ברמת הרשת — `api.pexels.com`,
   `images.pexels.com` ו-`www.pexels.com` מחזירים 403 על CONNECT. גם עם
   מפתח אי אפשר להתחבר משם.

המנגנון בנוי ומוכן. אצלך, עם גישה לרשת:

```bash
# 1. מפתח חינמי: https://www.pexels.com/api/
echo "PEXELS_API_KEY=המפתח_האמיתי" > .env      # הקובץ ב-gitignore

# 2. לוודא חיבור — בודק גם תמונות וגם וידאו
npm run pexels:test

# 3. לחפש: 10 שאילתות באנגלית על פני שמונת הפרקים
npm run pexels:search

# 4. לבחור: לפתוח את דפי Pexels שהודפסו, לבחור נכסים עם אותה תאורה
#    ואותה פלטה, ולרשום את המזהים ב-pexels.picks.json

# 5. להוריד ולחבר
npm run pexels:fetch
```

`pexels:fetch` מוריד את מה שבחרת, ממיר תמונות ל-WebP בארבעה רוחבים,
שומר וידאו כ-MP4 עם poster, וכותב את `src/data/pexelsAssets.ts`. הרצף
קורא את הקובץ הזה אוטומטית — נכס Pexels גובר על תמונת ברירת המחדל של
אותו פרק, והפוטר מתחיל להציג קרדיט לצלמים.

**המפתח לא מגיע לדפדפן.** זה אתר סטטי בלי שרת, ולכן הבקשה קורית בזמן
build ב-Node ורק קובצי התמונה נשלחים ללקוח. Vite חושף רק משתנים עם
הקידומת `VITE_`, ולכן `PEXELS_API_KEY` לא יכול לדלוף לבאנדל גם בטעות.
**אין לשנות את שמו ל-`VITE_PEXELS_API_KEY`.**
